...
class AutoTempa:
    def __init__(self):
        ...
        # SETTINGS
        self.atb_purchase_qty = 9  # qty atb per beli barang
        self.atb_per_purchase = 4  # qty atb yang digunakan
        self.wrs_brs_diamond_per_purchase = 5  # wrs diamond sekali beli
        self.pd_grs_per_purchase = 5  # pd grs sekali beli
        self.max_tempa = 11  # target tempa (kalo 11 sampe +11 klo 12 sampe +12)
    ...
    def open_refine(self):
        print(seal.get_freeze_dialog() == DIALOG['refine'])
        print(seal.get_freeze_dialog() == DIALOG['transaction'])
        print(seal.get_freeze_dialog())
        while seal.get_freeze_dialog() == 0:
            seal.moveMouse(383, 301)
            sleep(0.2)
            macro.mouseClick()

        while seal.get_freeze_dialog() == DIALOG['transaction']:
            macro.mouseClick()
    ...
    def get_item_from_bank(self, qty):
        ...
        while seal.get_freeze_dialog() == DIALOG['bank']:
